...
const COLUMNS = [
  ...
  { header: '검색 소요', key: 'searchMs', width: 22 },
  { header: '텍스트 수집', key: 'textCrawlMs', width: 22 },
  { header: '셀레니움 부팅', key: 'seleniumBootMs', width: 22 },
...
function buildRowValues(entry: CrawlLogEntry): Record<string, string | number> {
  ...
  return {
    ...
    searchMs: formatOptionalDuration(entry.searchMs),
    textCrawlMs: formatOptionalDuration(entry.textCrawlMs),
    seleniumBootMs: formatOptionalDuration(entry.seleniumBootMs),
