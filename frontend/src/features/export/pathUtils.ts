...
/** 크롤링 1회의 최상위 결과물 폴더. 하위에 연번 범위별 shard 폴더가 들어간다. */
export function buildResultFolderName(stamp: string): string {
  return `결과물_${stamp}`;
}

/** shard 폴더 이름. '연번 시작~연번 종료' 형식(예: 1~200). */
export function buildSerialRangeFolderName(startSerial: number, endSerial: number): string {
