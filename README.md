...
3. **크롤링 시작** 버튼을 클릭합니다. (텍스트 파싱 + 전체 페이지 스크린샷)
4. 지정한 폴더에 `연번 001_post_9295.png` 형식으로 캡처가 저장됩니다.
   - `001` — 엑셀 **연번**(데이터 행 번호, 1부터)
   - `9295` — URL의 `&no=9295` 게시글 번호
5. 결과 확인 후 **엑셀 다운로드** 버튼으로 `범죄일람표_YYYYMMDD_HHMM.xlsx` 파일을 브라우저로 받고, 같은 저장 디렉토리에도 저장됩니다.
...
| 작성 형태 | 게시글 / 게시글 + 댓글 |
| 원글 내용 또는 제목 | 게시글 제목 |
| 내용 | [issue], [body], [comments] 구조 |
| 죄명 | 수동 입력용 (빈 값) |
| 비고 | 조회수, 댓글 수, 캡처 상대 경로 등 |
| 연번표시 캡처파일 | 서버에 저장된 캡처 파일 절대 경로 |

## 스크린샷

- 저장 위치: **폴더 선택**으로 지정한 PC 로컬 폴더 (캡처는 서버에서 생성 후 해당 폴더로 복사)
