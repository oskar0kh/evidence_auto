...
2. **크롤링 시작** 버튼을 클릭합니다. (텍스트 파싱 + 전체 페이지 스크린샷)
3. `backend/captured_page/` 폴더에 `연번 001_post_9295.png` 형식으로 캡처가 저장됩니다.
   - `001` — 엑셀 **연번**(데이터 행 번호, 1부터)
   - `9295` — URL의 `&no=9295` 게시글 번호
4. 결과 확인 후 **엑셀 다운로드** 버튼으로 `범죄일람표_YYYYMMDD_HHMM.xlsx` 파일을 받습니다.
...
- 저장 위치: `backend/captured_page/` (설정: `evidence.screenshot-dir`)
